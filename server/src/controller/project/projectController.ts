        ...
        const stats = await Error.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    authCount: {
                        $sum: {
                            $cond: { if: { $or: [ { $eq: ["$_id", 401] }, { $eq: ["$_id", 403] } ] }, then: "$count", else: 0 }
                        }
                    },
                    notFoundCount: {
                        $sum: {
                    ...
                    mostCommonError: { $max: { count: "$count", code: "$_id", route: "$mostCommonRoute", message: "$mostCommonMessage" } }
                }
            }
        ]);

        if (stats.length === 0) {
            return next(new CustomError(404, "No errors found"));
        }

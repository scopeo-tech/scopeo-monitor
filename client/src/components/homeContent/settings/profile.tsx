...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const delayCheck = setTimeout(async () => {
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          username: response.data ? response.message : "",
        }));
      } catch {
  ...
  const handleSaveChanges = () => {
    ...
    if (hasChangedUsername && !username.trim())
  ...
  const isSubmitDisabled =
    (!hasChangedUsername && !newPassword) || !currentPassword;
  ...
  return (
    <div className="w-1/2">
      ...
      <div className="mb-6">
        ...
        <label className="block text-gray-600 font-normal mt-3">
          New Password
        </label>
        <input
          type="password"
      ...
      <button
        ...
        className={`mt-4 px-4 py-2 rounded-md ${
          isSubmitDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-green-600"
        }`}
      ...
      <div className="mt-8 p-2 border-t">
        ...
        <p className="text-gray-600 text-sm mb-3">
          Once you delete your profile, it will be deactivated immediately and
          all associated data will be permanently removed within approximately
